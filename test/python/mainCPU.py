...
def bench_mlir_matmul_f32(M, N, K, repeat):
    ...
    A_mem = fcg_memref_alloc_f32(M, K)
    B_mem = fcg_memref_alloc_f32(K, N)
    out_mem = fcg_memref_alloc_f32(M, N)
    fcg_memref_from_list_f32(A_mem, A_list)
    fcg_memref_from_list_f32(B_mem, B_list)
    fcg_memref_zero_f32(out_mem)

    C_cpu = matmul_cpu(A_list, B_list)
    fcg_matmul_f32(A_mem, B_mem, out_mem)
    C_mlir = fcg_memref_to_list_f32(out_mem)
...
def bench_mlir_add_f32(M, N, repeat):
    ...
    A_mem = fcg_memref_alloc_f32(M, N)
    B_mem = fcg_memref_alloc_f32(M, N)
    out_mem = fcg_memref_alloc_f32(M, N)
    fcg_memref_from_list_f32(A_mem, A_list)
    fcg_memref_from_list_f32(B_mem, B_list)
    fcg_memref_zero_f32(out_mem)
    ...
    fcg_add_f32(A_mem, B_mem, out_mem)
    C_mlir = fcg_memref_to_list_f32(out_mem)
...
def bench_mlir_relu_f32(M, N, repeat):
    ...
    X_mem = fcg_memref_alloc_f32(M, N)
    out_mem = fcg_memref_alloc_f32(M, N)
    fcg_memref_from_list_f32(X_mem, X_list)
    fcg_memref_zero_f32(out_mem)

    Y_cpu = relu_cpu(X_list)
    fcg_relu_f32(X_mem, out_mem)
    Y_mlir = fcg_memref_to_list_f32(out_mem)
...
def bench_mlir_full_flow_f32(M, N, K, repeat):
    ...
    A_mem = fcg_memref_alloc_f32(M, K)
    B_mem = fcg_memref_alloc_f32(K, N)
    bias_mem = fcg_memref_alloc_f32(M, N)
    out_mem = fcg_memref_alloc_f32(M, N)
    fcg_memref_from_list_f32(A_mem, A_list)
    fcg_memref_from_list_f32(B_mem, B_list)
    fcg_memref_from_list_f32(bias_mem, bias_list)
    fcg_memref_zero_f32(out_mem)

    C_cpu = full_flow_cpu(A_list, B_list, bias_list)
    fcg_full_flow_f32(A_mem, B_mem, bias_mem, out_mem)
    C_mlir = fcg_memref_to_list_f32(out_mem)
...
def bench_mlir_fusion_full_flow_f32(M, N, K, repeat):
    ...
    C_cpu = full_flow_cpu(A_list, B_list, bias_list)
    fcg_fusion_full_flow_f32(A_mem, B_mem, bias_mem, out_mem)
    C_mlir = fcg_memref_to_list_f32(out_mem)
...
def bench_mlir_matmul_f16(M, N, K, repeat):
    ...
    A_mem = fcg_memref_alloc_f16(M, K)
    B_mem = fcg_memref_alloc_f16(K, N)
    out_mem = fcg_memref_alloc_f16(M, N)
    fcg_memref_from_list_f16(A_mem, A_list)
    fcg_memref_from_list_f16(B_mem, B_list)
    fcg_memref_zero_f16(out_mem)

    C_cpu = matmul_cpu(A_list, B_list)
    fcg_matmul_f16(A_mem, B_mem, out_mem)
    C_mlir = fcg_memref_to_list_f16(out_mem)
...
def bench_mlir_add_f16(M, N, repeat):
    ...
    A_mem = fcg_memref_alloc_f16(M, N)
    B_mem = fcg_memref_alloc_f16(M, N)
    out_mem = fcg_memref_alloc_f16(M, N)
    fcg_memref_from_list_f16(A_mem, A_list)
    fcg_memref_from_list_f16(B_mem, B_list)
    fcg_memref_zero_f16(out_mem)
    ...
    C_cpu = add_cpu(A_list, B_list)
    fcg_add_f16(A_mem, B_mem, out_mem)
    C_mlir = fcg_memref_to_list_f16(out_mem)
...
def bench_mlir_relu_f16(M, N, repeat):
    ...
    X_mem = fcg_memref_alloc_f16(M, N)
    out_mem = fcg_memref_alloc_f16(M, N)
    fcg_memref_from_list_f16(X_mem, X_list)
    fcg_memref_zero_f16(out_mem)

    Y_cpu = relu_cpu(X_list)
    fcg_relu_f16(X_mem, out_mem)
    Y_mlir = fcg_memref_to_list_f16(out_mem)
...
def bench_mlir_full_flow_f16(M, N, K, repeat):
    ...
    A_mem = fcg_memref_alloc_f16(M, K)
    B_mem = fcg_memref_alloc_f16(K, N)
    bias_mem = fcg_memref_alloc_f16(M, N)
    out_mem = fcg_memref_alloc_f16(M, N)
    fcg_memref_from_list_f16(A_mem, A_list)
    fcg_memref_from_list_f16(B_mem, B_list)
    fcg_memref_from_list_f16(bias_mem, bias_list)
    fcg_memref_zero_f16(out_mem)

    C_cpu = full_flow_cpu(A_list, B_list, bias_list)
    fcg_full_flow_f16(A_mem, B_mem, bias_mem, out_mem)
    C_mlir = fcg_memref_to_list_f16(out_mem)
...
def bench_mlir_fusion_full_flow_f16(M, N, K, repeat):
    ...
    C_cpu = full_flow_cpu(A_list, B_list, bias_list)
    fcg_fusion_full_flow_f16(A_mem, B_mem, bias_mem, out_mem)
    C_mlir = fcg_memref_to_list_f16(out_mem)
...
def main():
    parser = argparse.ArgumentParser(description='Performance Comparison: CPU vs MLIR')
